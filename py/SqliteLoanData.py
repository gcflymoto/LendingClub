...
class SqliteLCLoanData(LoanData.LCLoanData):
    ...
    def mid_stage_initialize(self):
        self.info_msg("Initializing sqlite3 into memory")
        self.create_sqlite_db(self.loans)

    def init_sqlite_db(self, db_name):
    ...
    def open_sqlite_db(self):
        db_name = "loans_serialized_python%d.db" % sys.version_info.major
        self.info_msg("Initializing sqlite3 from %s ..." % db_name)

        if utilities.check_for_sqlite():
            self.con = self.init_sqlite_db(db_name)
        else:
            self.con = sqlite3.connect(db_name)
        self.cursor = self.con.cursor()
        self.info_msg("Initializing sqlite3 from %s done ..." % db_name)