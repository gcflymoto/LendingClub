"""
Created on May 30, 2013

@author:     Gregory Czajkowski

@copyright:  2013 Freedom. All rights reserved.

@license:    Licensed under the Apache License 2.0 http://www.apache.org/licenses/LICENSE-2.0

@contact:    gregczajkowski at yahoo.com
"""

# These enums are used for conversions and backtesting
LOAN_ENUM_rowid = 0
LOAN_ENUM_open_acc = 1
LOAN_ENUM_funded_amnt = 2
LOAN_ENUM_annual_income = 3
LOAN_ENUM_grade = 4
LOAN_ENUM_debt_to_income_ratio = 5
LOAN_ENUM_delinq_2yrs = 6
LOAN_ENUM_earliest_credit_line = 7
LOAN_ENUM_emp_length = 8
LOAN_ENUM_home_ownership = 9
LOAN_ENUM_income_validated = 10
LOAN_ENUM_inq_last_6mths = 11
LOAN_ENUM_purpose = 12
LOAN_ENUM_mths_since_last_delinq = 13
LOAN_ENUM_pub_rec = 14
LOAN_ENUM_revol_utilization = 15
LOAN_ENUM_addr_state = 16
LOAN_ENUM_total_acc = 17
LOAN_ENUM_desc_word_count = 18
LOAN_ENUM_term = 19

# These enums are only used for conversions

LOAN_ENUM_loan_status = 20
LOAN_ENUM_issue_datetime = 21
LOAN_ENUM_installment = 22
LOAN_ENUM_int_rate = 23
LOAN_ENUM_total_pymnt = 24
LOAN_ENUM_out_prncp = 25
LOAN_ENUM_out_prncp_inv = 26
LOAN_ENUM_profit = 27
LOAN_ENUM_principal = 28
LOAN_ENUM_lost = 29
LOAN_ENUM_defaulted = 30

LOAN_ENUM_SIZE = 31
